 ...
 *
 * @author khinds, dschmitz
 * @license http://opensource.org/licenses/gpl-license.php GNU Public License
 * @copyright Shamanic, http://www.shamanic.io
 */


exports.getMapDistances = function(req) {
  return new req.promise(function (resolve, reject) {
    req.db.fetchAll('SELECT map_data FROM maps where user_uuid = ?', [req.session.user.uuid], function(err, result) {
      if(err) {
        return reject(err);
      }
